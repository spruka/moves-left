...
std::vector<std::string> Search::GetVerboseStats(Node* node,
  ...
  std::sort(
      ...
      [&fpu, &U_coeff, &drawScore, &logit_q](EdgeAndNode a, EdgeAndNode b) {
        return std::forward_as_tuple(a.GetN(), a.GetQD(fpu, drawScore, logit_q) + a.GetU(U_coeff)) <
               std::forward_as_tuple(b.GetN(), b.GetQD(fpu, drawScore, logit_q) + b.GetU(U_coeff));
      });
